Meteor.myFunctions = {
	renderMeuN : function () {
        ...
        _.each(atividadesTotal, function(atividade) {
        	...
        	if (Meteor.user().profile.activities.indexOf(userId) != -1) {
			    ...
			    thisItem += "<p><strong>" + atividade.nomeministrante + "</strong></p>";
			    thisItem += "<p>" + atividade.local +" "+ atividade.horarioinicio +" - Duração: "+ atividade.duracao +"</p>";
			    thisItem += "<p>"+ atividade.tipo + " - "+ atividade.linhas + " - Participantes: " + atividade.participantes + "</p>";
    ...
    if (typeof atividadesByTag === 'undefined') {
      ...
      var toRender = "<li class='collection-item'>Vá para <a href='/tags'>Lista de Linhas</a> para filtrar as atividades pelas linhas do N.</li>"

    } else {

      var atividadesTotal = Atividades.find({
        'linhas': atividadesByTag.toString()
      }, {sort: {datainicio: 1}}).fetch();

      if (atividadesTotal == '') {

        var toRender = "<li class='collection-item'>Nenhuma atividade encontrada.</li><li class='collection-item'><a href='/tags' class='btn waves-effect waves-light'>Voltar para Lista de Linhas</a></li>"
        ...
        var currentTime = "";
          ...
          _.each(atividadesTotal, function(atividade) {
            ...
            if (currentTime !== atividade.datainicio) {
              thisItem += "<p class='horario'>"+ atividade.datainicio +"/07</p>";
            };
            ...
            thisItem += "<span class='title'>" + atividade.titulo + "</span>";
            thisItem += "<p><strong>" + atividade.nomeministrante + "</strong></p>";
            thisItem += "<p>" + atividade.local +", Data: "+ atividade.datainicio + "/07 - "+ atividade.horarioinicio +", Duração: "+ atividade.duracao +"</p>";
            thisItem += "<p>"+ atividade.tipo + ", Linhas: "+ atividade.linhas + "</p>";
